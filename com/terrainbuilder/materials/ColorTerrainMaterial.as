...
package com.terrainbuilder.materials
{
	...
	import com.terrainbuilder.materials.passes.ColorTerrainPass;
	...
	public class ColorTerrainMaterial extends MaterialBase
	{
		...
		private var _splatDatas:Vector.<ColoredSplatData>;
		private var _splatDataGroups:Vector.<Vector.<ColoredSplatData>> = new Vector.<Vector.<ColoredSplatData>>();
		...
		public function ColorTerrainMaterial(squaredTiles:uint, seamlessElevationData:SeamlessElevationData, anchorYPosition:Number, coloredSplatDatas:Vector.<ColoredSplatData>, tileData:Number = 4, smooth:Boolean = true, repeat:Boolean = false, mipmap:Boolean = true)
		{
			
			_coloredSplatDatas = coloredSplatDatas;
			
			for (var i:uint = 0; i<_coloredSplatDatas.length; i++) {
				
				if (_coloredSplatDatas[i].normalMap !== null) {
			...
			if (coloredSplatDatas.length < 1) throw Error("ColorMultiPassTerrain material requires at least one splat");
			
			for (i = 0; i<_coloredSplatDatas.length; i++) {
				
				if (i % _maxSplatDatasPerPass == 0) {
					_splatDatas = new Vector.<ColoredSplatData>();
					_splatDataGroups.push(_splatDatas);
				}
				
				_splatDatas.push(_coloredSplatDatas[i]);
			}
			...
			for (i = 0; i<_splatDataGroups.length; i++) {
				...
				addPass(new ColorTerrainPass(_splatDataGroups[i], _terrainMethodData, blendingRequired));
			}
		}
		...
		public function updateBlend(blend:BitmapData):void {
			...
			for (var i:uint = 0; i<_passes.length; i++) {
				if (_passes[i] is ColorTerrainPass) {
					var pass:ColorTerrainPass = ColorTerrainPass(_passes[i]);
					for (var j:uint = 0; j<pass.coloredSplatDatas.length; j++) {
						if (pass.coloredSplatDatas[j].blend == blend) {
		...
		public function updatePasses(clearTextures:Boolean = true):void {
			...
			if (_useNormalMaps) {
				...
				var visibleColoredSplatDatas:Vector.<ColoredSplatData> = new Vector.<ColoredSplatData>();
				for (i = 0; i<_coloredSplatDatas.length; i++) {
					if (_coloredSplatDatas[i].visible) {
						visibleColoredSplatDatas.push(_coloredSplatDatas[i]);
					}
				}
				
				_splatDataGroups = new Vector.<Vector.<ColoredSplatData>>();
				for (i = 0; i<visibleColoredSplatDatas.length; i++) {
					...
					_splatDatas.push(visibleColoredSplatDatas[i]);
				}
				...
				for (i = 0; i<_splatDataGroups.length; i++) {
					...
					addPass(new ColorTerrainPass(_splatDataGroups[i], _terrainMethodData, blendingRequired, true, fogInPass));
				}
			} else {
				_maxSplatDatasPerPass = 6;
				
				visibleColoredSplatDatas = new Vector.<ColoredSplatData>();
				for (i = 0; i<_coloredSplatDatas.length; i++) {
					if (_coloredSplatDatas[i].visible) {
						visibleColoredSplatDatas.push(_coloredSplatDatas[i]);
					}
				...
				for (i = 0; i<visibleColoredSplatDatas.length; i++) {
					if (visibleColoredSplatDatas[i].useSlopeBlend) {
						useSlopeCount++;
						if (useSlopeCount == 3) { 
				...
				for (i = 0; i<_splatDataGroups.length; i++) {
					...
					addPass(new ColorTerrainPass(_splatDataGroups[i], _terrainMethodData, blendingRequired, false, fogInPass));
				}
			}
		...
		public function set coloredSplatDatas(value:Vector.<ColoredSplatData>):void { _coloredSplatDatas = value; }
